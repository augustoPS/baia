...
@Suite struct GitStatusParserTests {
    ...
    @Test func readsACleanBranchWithNoUpstream() {
        let status = GitStatusParser.parse(PorcelainFixture.zeroed("""
    ...
    @Test func countsARenameWhoseOriginalPathIsItsOwnEntry() {
        // Captured verbatim from `git mv` followed by an edit. Under `-z` the
        // original path is a NUL terminated entry of its own rather than a field
        // after a tab, so it appears here as the line below the record. Counting
        // it as a record of its own is the failure this pins: it would add a
        // phantom change for every rename.
        let status = GitStatusParser.parse(PorcelainFixture.zeroed("""
        # branch.oid ed1d41d21c1540a4b34d59e9a720d4f4383ed3cd
        # branch.head main
        2 RM N... 100644 100644 100644 78981922613b2afb6025042ff6bd878ac1994e85 78981922613b2afb6025042ff6bd878ac1994e85 R100 new name.txt
        old name.txt
        """))
        #expect(status?.staged == 1)
        #expect(status?.unstaged == 1)
    ...
    @Test func treatsANewlineAsAPathCharacterRatherThanASeparator() {
        // The inversion `-z` brings. Records end at a NUL and nowhere else, so a
