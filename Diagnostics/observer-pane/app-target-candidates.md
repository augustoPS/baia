...
| ~~`Sources/ConfigurationCenter.swift:61-150`~~ | ~~`themeDefinition(from:)`, `terminalTheme(from:)`, `terminalConfiguration(from:)`, `paneTheme(from:)`~~ | **Moved 2026-08-03 to `PaneChrome.SettingsDerivations` (`9e60308`).** The caveat was right and the conclusion was wrong: no *fourth* package was needed, because `PaneChrome` already owned `PaneTheme` and both non-ghostty dependencies, so it only had to take one more. The real question was whether a package can depend on libghostty without forfeiting the headless `make test` path, and it was settled by building a throwaway package rather than by reading manifests: it can. `GhosttyTerminal` had to come too, which this row did not anticipate, since `TerminalTheme` and `TerminalConfiguration` are defined there rather than in `GhosttyTheme`; measured again with it, still headless. `derivations(for:)` and `chrome(for:)` stayed behind as one-line wrappers, since both read `self.settings`. |
